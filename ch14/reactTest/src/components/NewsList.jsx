...
const NewsList = ({ category }) => {
  const sendData = () => {
    if (category === 'daegu') {
      return axios.get(`/kor/api/tasty.html?mode=json&addr=중구`);
      ...
      return axios.get(`/gimhae/api/restaurant.do`);
  ...
  return (
    <NewsListBlock>
      {category === 'daegu'
        ...
        : category === 'globalnews'
        ? data.map((item, index) => <GlobalNews key={index} article={item} />)
        : category === 'gimhae'
        ? data.map((item, index) => <Gimhae key={index} article={item} />)
        : category === 'busanAtt'
        ? data.map((item, index) => <PdItemBusan key={index} article={item} />)
        : data.map((item) => <Daegu key={item.seq} article={item} />)}
    </NewsListBlock>
  );
};
